...
impl StereoCamera {
    pub fn new(st: StereoCameraType, o: &Point, t: &Point, u: &Vector) -> Self {
        let inner = PerspectiveCamera::new(CameraType::Stereo, o, t, u, None);
...
impl Camera for StereoCamera {
    ...
    fn get_raster_to_camera(&self, idx: u32) -> Option<&Transform> {
        if idx == 0 {
            self.left_eye.as_ref().unwrap().get_raster_to_camera(0)
        } else if idx == 1 {
            self.right_eye.as_ref().unwrap().get_raster_to_camera(0)
        } else {
    ...
    fn get_camera_to_world(&self, idx: u32) -> Option<&Transform> {
        if idx == 0 {
            self.left_eye.as_ref().unwrap().get_camera_to_world(0)
        } else if idx == 1 {
            self.right_eye.as_ref().unwrap().get_camera_to_world(0)
        } else {
    ...
    fn get_screen_to_world(&self, idx: u32) -> Option<&Transform> {
        if idx == 0 {
            self.left_eye.as_ref().unwrap().get_screen_to_world(0)
        } else if idx == 1 {
            self.right_eye.as_ref().unwrap().get_screen_to_world(0)
        } else {
    ...
    fn update(&mut self, film_width: u32, film_height: u32, film_sub_region: Option<[u32; 4]>) {
        ...
        match self.stereo_type {
            StereoCameraType::StereoPerspective => {
                ...
                left.base_mut().clip_hither = self.base.clip_hither;
                left.base_mut().clip_yon = self.base.clip_yon;
                left.base_mut().shutter_open = self.base.shutter_open;
                left.base_mut().shutter_close = self.base.shutter_close;
                left.base_mut().auto_volume = self.base.auto_volume;
                left.base_mut().volume = self.base.volume.clone();

                left.base_mut().clipping_plane_center = self.base.clipping_plane_center.clone();
                left.base_mut().clipping_plane_normal = self.base.clipping_plane_normal.clone();
                left.base_mut().enable_clipping_plane = self.base.enable_clipping_plane;

                left.base_mut().lens_radius = self.base.lens_radius;
                left.base_mut().focal_distance = self.base.focal_distance;
                left.base_mut().auto_focus = self.base.auto_focus;

                left.base_mut().screen_offset_x = -self.horiz_stereo_lens_distance * 0.5;
                left.base_mut().field_of_view = self.base.field_of_view;
                left.base_mut().enable_oculus_rift_barrel = self.base.enable_oculus_rift_barrel;

                left.base_mut().update(film_width / 2, film_height, None);
                ...
                right.base_mut().clip_hither = self.base.clip_hither;
                right.base_mut().clip_yon = self.base.clip_yon;
                right.base_mut().shutter_open = self.base.shutter_open;
                right.base_mut().shutter_close = self.base.shutter_close;
                right.base_mut().auto_volume = self.base.auto_volume;
                right.base_mut().volume = self.base.volume.clone();

                right.base_mut().clipping_plane_center = self.base.clipping_plane_center.clone();
                right.base_mut().clipping_plane_normal = self.base.clipping_plane_normal.clone();
                right.base_mut().enable_clipping_plane = self.base.enable_clipping_plane.clone();

                right.base_mut().lens_radius = self.base.lens_radius;
                right.base_mut().focal_distance = self.base.focal_distance;
                right.base_mut().auto_focus = self.base.auto_focus;

                right.base_mut().screen_offset_x = self.base.screen_offset_x;
                right.base_mut().field_of_view = self.base.field_of_view;
                right.base_mut().enable_oculus_rift_barrel = self.base.enable_oculus_rift_barrel;

                right.base_mut().update(film_width / 2, film_height, None);
            ...
            StereoCameraType::StereoEnvironment180 => {
                ...
                right.base_mut().screen_offset_x = self.horiz_stereo_lens_distance * 0.5;
                right.base_mut().degrees = 180.0;
                right.base_mut().update(film_width / 2, film_height, None);
                self.right_eye = Some(Box::new(right));
            },
            StereoCameraType::StereoEnvironment360 => {
                ...
                left.base_mut().screen_offset_x = -self.horiz_stereo_lens_distance * 0.5;
                left.base_mut().degrees = 360.0;
                left.base_mut().update(film_width, film_height / 2, None);
                self.left_eye = Some(Box::new(left));
                ...
                right.base_mut().screen_offset_x = self.horiz_stereo_lens_distance * 0.5;
                right.base_mut().degrees = 360.0;
                right.base_mut().update(film_width, film_height / 2, None);
                self.right_eye = Some(Box::new(right));
            },
        }
    }
    ...
    fn sample_lens(&self, time: f32, u1: f32, u2: f32, lens_point: &mut Point) -> bool {
        self.left_eye
            .as_ref()
            .unwrap()
            .sample_lens(time, u1, u2, lens_point)
    }
