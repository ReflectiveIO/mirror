...
pub struct SubtractTexture {
    tex1: Box<dyn Texture>,
    tex2: Box<dyn Texture>,
}

impl SubtractTexture {
...
impl Texture for SubtractTexture {
    fn get_type(&self) -> TextureType { TextureType::SubtractTex }

    fn get_float_value(&self, hp: &HitPoint) -> f32 { todo!() }

    fn get_spectrum_value(&self, hp: &HitPoint) -> Spectrum { todo!() }

    fn y(&self) -> f32 { self.tex1.y() - self.tex2.y() }

    fn filter(&self) -> f32 { self.tex1.filter() - self.tex2.filter() }
