...
/// Film stores all the outputs of a rendering. It can be obtained from
/// RenderSession or as stand alone object loaded from a file.
...
impl Film {
    ...
    /// Loads a stand alone Film (i.e. not connected to a rendering session)
    /// from a file.
    pub fn load(filename: &str) -> Film { Film::new() }

    /// Create a stand alone Film (i.e. not conncted to a rendering session)
    /// from the properties.
    ///
    /// * props defining the film.
    /// * has_pixel_normalized_channel if the film must have
    ///   ChannelRadiancePerPixelNormalized.
    /// * has_screen_normalized_channel if the film must have
    ...
    /// Returns a list of statistics related to the film. Mostly useful for
    /// stand alone films.
    pub fn stats(&self) -> Properties { Properties::default() }

    /// Returns the Film average luminance. It can be use to estimate a good
    /// value for variance clamping.
    pub fn y(&self) -> f64 { 0 as f64 }
    ...
    pub fn save_outputs(&self) { trace!("@TODO: saves all film output channels") }
    ...
    pub fn total_sample_count(&self) -> f64 { 0 as f64 }

    /// Return the size (in f64 or u64) of a File output channel.
    pub fn output_size(&self) -> isize { 0 }

    /// Returns if a film channel output is available.
    pub fn has_output(&self, output_type: FilmOutput) -> bool { false }

    /// Returns the number of output channels of the passed type.
    pub fn output_count(&self, output_type: FilmOutput) -> u32 { 0 as u32 }
}
